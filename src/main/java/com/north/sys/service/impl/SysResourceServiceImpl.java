...
public class SysResourceServiceImpl extends ServiceImpl<SysResourceMapper, SysResource> implements ISysResourceService {
    ...
    @Override
    public List<TreeDto<SysResource>> getSysResourceTreeList(String userId, Integer resourceType, String pid) {
        List<SysResource> list = sysUserMapper.getUserResource(userId);

        List<TreeDto<SysResource>> treeList = new ArrayList<>();
        for (SysResource resource : list) {
            if (resourceType == null || resource.getResourceType().equals(resourceType)) {
